...
export class ProgrammeService {
  ...
  async listCohorts(organizationId: string, programmeId: string): Promise<Cohort[]> { const { data, error } = await supabase.from('institutional_programme_cohorts').select('*').eq('organization_id', organizationId).eq('programme_id', programmeId).order('created_at', { ascending: false }); if (error) throw error; return (data ?? []).map(this.mapCohort); }
  async listBenefits(organizationId: string, programmeId: string): Promise<Benefit[]> { const { data, error } = await supabase.from('institutional_programme_benefits').select('*').eq('organization_id', organizationId).eq('programme_id', programmeId).order('created_at', { ascending: false }); if (error) throw error; return (data ?? []).map(this.mapBenefit); }
