...
import Joi from 'joi';
import { ApproveInvestmentAllocationPlanCommand,CreateInvestmentAllocationPlanCommand,ExecuteInvestmentAllocationPlanCommand,InvestmentAllocationPlanService,InvestmentAllocationPlanValidationError,RecognizeInvestmentRefundObligationsCommand,investmentAllocationPlanService } from '../domains/financial/investmentAllocationPlanService.js';
...
const executeSchema=recognizeRefundsSchema;
export class InvestmentAllocationPlanController {
  ...
  execute=async(req:TenantRequest,res:Response)=>{const v=this.validate<ExecuteInvestmentAllocationPlanCommand>(executeSchema,req.body,res);if(!v)return;try{return res.status(201).json(await this.service.execute({...v,organizationId:req.tenant!.id,actorId:req.user!.id,planId:req.params.planId}));}catch(e){return this.error(e,res);}};
