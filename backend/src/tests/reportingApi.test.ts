...
jest.mock("../services/reportingService.js", () => ({
  REPORT_EXPORT_DISABLED: 'Organization report exports are disabled',
  ReportingService: {
    getBookingReport: jest.fn(),
    getEngagementReport: jest.fn(),
    getRetentionBI: jest.fn(),
    exportToCSV: jest.fn(),
  },
}));
...
describe('reporting tenant API', () => {
  ...
  it('passes the resolved tenant to enabled exports and returns CSV', async () => {
  ...
  it('returns forbidden when the tenant export policy is disabled', async () => {
    (ReportingService.exportToCSV as jest.Mock).mockRejectedValue(
      new Error('Organization report exports are disabled'),
    );
    await request(app)
      .post('/api/reports/export')
      .send({ table: 'bookings', fields: ['id'] })
      .expect(403, { error: 'Organization report exports are disabled' });
  });
});
