BEGIN;
DO $$
DECLARE owner UUID; reader UUID; denied UUID; foreign_owner UUID; org UUID; foreign_org UUID; cash UUID; equity UUID; foreign_cash UUID; foreign_equity UUID; result JSONB;
BEGIN
 ...
 INSERT INTO users(email,password,name,role) VALUES('ac07-foreign-owner-'||replace(gen_random_uuid()::TEXT,'-','')||'@example.test','test','AC07 Foreign Owner','farmer') RETURNING id INTO foreign_owner;
 ...
 INSERT INTO organizations(name,slug,type,created_by) VALUES('AC07 Foreign Tenant','ac07-foreign-'||replace(gen_random_uuid()::TEXT,'-',''),'cooperative',foreign_owner) RETURNING id INTO foreign_org;
 ...
 INSERT INTO organization_memberships(organization_id,user_id,role,status,permissions,joined_at) VALUES(foreign_org,foreign_owner,'owner','active','{}',NOW());
 ...
 INSERT INTO accounting_periods(organization_id,name,starts_on,ends_on,status) VALUES(foreign_org,'AC07 Foreign FY 2028',DATE '2028-01-01',DATE '2028-12-31','open');
 ...
 INSERT INTO financial_accounts(organization_id,code,name,account_class,normal_side,currency,owner_type,is_control,created_by) VALUES(foreign_org,'AC07.FOREIGN.CASH','AC07 foreign cash','asset','debit','NGN','organization',TRUE,foreign_owner) RETURNING id INTO foreign_cash;
 INSERT INTO financial_accounts(organization_id,code,name,account_class,normal_side,currency,owner_type,is_control,created_by) VALUES(foreign_org,'AC07.FOREIGN.EQUITY','AC07 foreign equity','equity','credit','NGN','organization',FALSE,foreign_owner) RETURNING id INTO foreign_equity;
 ...
 PERFORM post_financial_journal(foreign_org,'NGN',DATE '2028-02-01','audit.test','opening','ac07-foreign-entry',repeat('b',64),'00000000-0000-4000-8000-000000007002','AC07 foreign entry',foreign_owner,jsonb_build_array(jsonb_build_object('account_id',foreign_cash,'line_number',1,'side','debit','amount_minor',2000),jsonb_build_object('account_id',foreign_equity,'line_number',2,'side','credit','amount_minor',2000)));
 ...
 BEGIN PERFORM read_accounting_audit_export(foreign_org,reader,'NGN',DATE '2028-01-01',DATE '2028-12-31',clock_timestamp()); RAISE EXCEPTION 'cross-tenant audit export accepted'; EXCEPTION WHEN OTHERS THEN IF SQLERRM='cross-tenant audit export accepted' OR SQLERRM NOT LIKE '%ACCOUNTING_AUDIT_EXPORT_PERMISSION_DENIED%' THEN RAISE; END IF; END;
