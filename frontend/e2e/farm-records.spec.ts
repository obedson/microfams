...
test('farm records workflow creates and displays a tenant-scoped record', async ({ page }) => {
  ...
  await page.route('**/api/auth/login', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { user: { id: 'farmer-1', email: 'farmer@example.test', name: 'Test Farmer', role: 'farmer' }, token: 'test-token', refreshToken: 'refresh-token' } }) }));
