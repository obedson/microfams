-- CRD-10 database contract: maker-checker zero-fee loan write-off,
-- default eligibility, balanced loss posting, replay, isolation, and immutability.
...
DECLARE
  ...
  v_review_correlation UUID:=gen_random_uuid();
  v_failed BOOLEAN;
BEGIN
  ...
  INSERT INTO organization_memberships(
    ...
    ARRAY['financial.loans.configure','financial.loans.review',
      'financial.loans.disburse','financial.loans.service_existing','financial.accounts.manage'],
    'active',NOW()
  );
  INSERT INTO users(email,password,name,role) VALUES
  ...
  PERFORM assess_loan_delinquency(v_org,v_reviewer,v_application,v_contract.id,
    CURRENT_DATE+250,gen_random_uuid(),'crd10-default-assessment-001');
  v_result:=propose_loan_writeoff(v_org,v_applicant,v_application,v_contract.id,
    'UNCOLLECTIBLE_DEFAULT','Documented recovery attempts establish an uncollectible default.',
    jsonb_build_array('case:crd10:1'),v_correlation,'crd10-writeoff-propose-001','2026-08-13T15:00:00Z');
  v_writeoff:=(v_result->'writeoff'->>'id')::UUID;
  IF v_writeoff IS NULL OR v_result->'writeoff'->>'state'<>'proposed'
  THEN RAISE EXCEPTION 'CRD10: write-off proposal was not recorded'; END IF;

  IF (propose_loan_writeoff(v_org,v_applicant,v_application,v_contract.id,
      'UNCOLLECTIBLE_DEFAULT','Documented recovery attempts establish an uncollectible default.',
      jsonb_build_array('case:crd10:1'),v_correlation,'crd10-writeoff-propose-001','2026-08-13T15:00:00Z')
      ->'writeoff'->>'id')::UUID<>v_writeoff
  THEN RAISE EXCEPTION 'CRD10: proposal replay failed'; END IF;

  v_failed:=FALSE;
  BEGIN
    PERFORM decide_loan_writeoff(v_org,v_applicant,v_writeoff,'approve',
      'Maker cannot approve the same write-off proposal.',gen_random_uuid(),
      'crd10-maker-approve-001','2026-08-13T15:00:30Z');
  EXCEPTION WHEN OTHERS THEN
    IF SQLERRM LIKE '%independent%' THEN v_failed:=TRUE; END IF;
  END;
  IF NOT v_failed THEN RAISE EXCEPTION 'CRD10: maker approved own proposal'; END IF;

  v_result:=decide_loan_writeoff(v_org,v_reviewer,v_writeoff,'approve',
    'Independent review confirms exhaustion of reasonable recovery actions.',v_review_correlation,
    'crd10-approve-001','2026-08-13T15:01:00Z');
  v_loss_journal:=(v_result->'writeoff'->>'loss_journal_entry_id')::UUID;
  SELECT account_id INTO v_loss_account FROM journal_lines
    WHERE journal_entry_id=v_loss_journal AND side='debit';
  IF v_loss_journal IS NULL OR (SELECT state FROM loan_contracts WHERE id=v_contract.id)<>'written_off'
    OR (SELECT state FROM loan_applications WHERE id=v_application)<>'written_off'
    OR (SELECT SUM(CASE WHEN side='debit' THEN amount_minor ELSE 0 END) FROM journal_lines WHERE journal_entry_id=v_loss_journal)<>v_contract.principal_original_minor
    OR (SELECT SUM(CASE WHEN side='credit' THEN amount_minor ELSE 0 END) FROM journal_lines WHERE journal_entry_id=v_loss_journal)<>v_contract.principal_original_minor
    OR (SELECT purpose FROM financial_accounts WHERE id=v_loss_account)<>'credit_loss_writeoff'
  THEN RAISE EXCEPTION 'CRD10: approved write-off did not reconcile'; END IF;
  IF (decide_loan_writeoff(v_org,v_reviewer,v_writeoff,'approve',
      'Independent review confirms exhaustion of reasonable recovery actions.',v_review_correlation,
      'crd10-approve-001','2026-08-13T15:01:00Z')
      ->'writeoff'->>'loss_journal_entry_id')::UUID<>v_loss_journal
  THEN RAISE EXCEPTION 'CRD10: decision replay failed'; END IF;

  IF NOT EXISTS(SELECT 1 FROM organization_audit_log WHERE organization_id=v_org
      AND action='LOAN_WRITEOFF_APPROVED' AND resource_id=v_writeoff::TEXT)
  THEN RAISE EXCEPTION 'CRD10: approval audit evidence is incomplete'; END IF;

  v_failed:=FALSE;
  BEGIN
    PERFORM propose_loan_writeoff(v_org,v_outsider,v_application,v_contract.id,
      'UNCOLLECTIBLE_DEFAULT','Outsider must not write off a tenant loan.',
      jsonb_build_array('case:outsider'),gen_random_uuid(),
      'crd10-outsider-rejected-001',NOW());
  EXCEPTION WHEN OTHERS THEN
    IF SQLERRM LIKE '%Missing financial.loans.service_existing permission%'
      THEN v_failed:=TRUE; END IF;
  END;
  IF NOT v_failed THEN RAISE EXCEPTION 'CRD10: outsider serviced a tenant loan'; END IF;

  IF has_table_privilege('service_role','public.loan_writeoffs','UPDATE')
  THEN RAISE EXCEPTION 'CRD10: service role can directly mutate write-offs'; END IF;
END $$;

ROLLBACK;

SELECT 'loan write-off schema tests passed' AS result;
