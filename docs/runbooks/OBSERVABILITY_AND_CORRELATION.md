...
When SENTRY_DSN is configured, the backend initializes Sentry before request routing so request context and tracing cover every API request. The Sentry error handler runs before the public error envelope. Without SENTRY_DSN, telemetry is disabled and the existing correlation, metrics, and health behavior remains active.

Shared audit writes inherit the active HTTP correlation identifier through async request context. Domain code may provide an explicit correlation identifier when continuing an existing workflow. Background and scheduled audit writes without request context receive a generated UUID, so operational evidence is never stored without a correlation key.