-- CRD-06 database contract: governed zero-fee repayment, journal posting,
-- replay, tenant/permission isolation, payoff reconciliation, and immutability.
...
BEGIN
  ...
  BEGIN
    PERFORM record_loan_repayment(
      v_org,v_reviewer,v_application,v_contract.id,1,
      CURRENT_DATE,gen_random_uuid(),'crd06-after-payoff-rejected-001'
    );
  EXCEPTION WHEN OTHERS THEN v_failed:=TRUE; END;
  IF NOT v_failed THEN RAISE EXCEPTION 'CRD06: repayment after payoff was accepted'; END IF;
...
SELECT 'loan repayment servicing schema tests passed' AS result;
