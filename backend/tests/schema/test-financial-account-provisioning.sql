...
BEGIN
 FOR rule IN SELECT * FROM financial_account_purpose_rules ORDER BY purpose LOOP
  ...
  END IF;
 END LOOP;
 IF counter<>30 THEN RAISE EXCEPTION 'canonical purpose catalogue is incomplete'; END IF;
 ...
 IF (SELECT count(*) FROM organization_audit_log WHERE action='FINANCIAL_ACCOUNT_PROVISIONED' AND organization_id=org)<>30 THEN RAISE EXCEPTION 'account provisioning audit count is incorrect'; END IF;
END $$;
